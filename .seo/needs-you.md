...
| NY-7 | 2026-09-12 | clean link audits | Cloudflare's email-obfuscation feature rewrites the plain `mailto:jason@spidleweb.net` in page source into `/cdn-cgi/l/email-protection` links at the edge, which health_diff sees as a broken link (404). Keep obfuscation on (spam protection; the audit flags it as a documented false positive) or turn it off in the Cloudflare dashboard (Scrape Shield → Email Address Obfuscation) for cleaner audits? | | |
